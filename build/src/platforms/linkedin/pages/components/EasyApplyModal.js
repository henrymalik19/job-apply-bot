...
class EasyApplyModal {
    ...
    constructor(page) {
        ...
        this.discardBtn = this.page.locator('[data-control-name="discard_application_confirm_btn"]');
    }
    async apply() {
        ...
        while (true) {
            ...
            if (await this.hasErrors()) {
                console.info("[info] unable to submit application -> missing information");
    ...
    async answerTextFieldQuestions() {
        try {
            ...
            for (const txtField of emptyTxtFields) {
                const labelTxt = await txtField.evaluate((n) => n.labels[0].textContent);
                await this.self.getByLabel(labelTxt).fill("10");
            }
        }
        catch (error) {
            console.error("[error] unable to answer text field questions. continuing...", error);
        }
    }
    async answerSelectQuestions() {
        try {
            ...
            // );
            for (const selectInput of selectInputs) {
                const labelTxt = await selectInput.evaluate((n) => n.labels[0].childNodes[1].textContent?.trim());
                const select = await this.self.getByLabel(labelTxt);
            ...
            console.error("[error] unable to answer select input questions. continuing...", error);
        }
    }
    async answerRadioQuestions() {
        try {
    ...
    async hasErrors() {
        const errors = await this.self
            ...
            .all();
        if (errors.length !== 0) {
            console.info(`[info] ${errors.length} fields found with missing information`);
        }
    ...
    async saveErrorsToDB() {
        ...
        for (const section of sections) {
            ...
            if (hasErrorMsg) {
                ...
                console.debug("[debug]", (await section.locator("label").textContent())?.trim());
            }
        }
        // await Promise.all(
...
exports.EasyApplyModal = EasyApplyModal;
